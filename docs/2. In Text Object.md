...
The ``.strip_to`` method creates a clone whose ``.span`` matches the extend of its children.  If no .children are
...
2. Elements edges correspond to the ``.stop`` of match spans; elements are all adjacent
