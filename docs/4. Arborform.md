...
Transformations are performed in the ``.transform`` method, which is abstract for the base class ``Itorator``.  Several concrete itorators with implementations for ``.transform`` are available:
...
You can implement new functionality by deriving a new class from ``Itorator`` and implementing ``.transform``.
...
Note that the return type for your wrapped transorformation must be an ito **sequence**.  Even if your transformation alters the input ito, or creates a single new ito, be sure your return value is still a sequence:

```python
>>> lambda i: i.str_strip()  # NOT a sequence!
>>> lambda i: (i.str_strip(),)  # Ok - returns an Ito tuple of length 1
```

Class ``Itorator`` also features a ``.tag`` property, which is a convenience field that allows you to assign arbirary labels in order to assist in testing and development of your pipelines.
...
I.e., each ``Ito`` in the sequence is passed to an ``Itorator`` which then **transforms** it via its ``.transform`` method.  For example, an ``Itorator`` that performs a word split can pass its output to another ``Itorator`` that performs a char split.  The overall output will consist of char ``Ito`` objects:
...
The order of evaluation for these properties is:

1. ``.transform`` : Ito → Sequence
