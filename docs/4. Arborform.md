...
Transformations are performed in the ``._transform`` method, which is abstract for the base class ``Itorator``.  Several concrete itorators with implementations for ``.transform`` are available:
