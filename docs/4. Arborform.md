...
The Pawpaw arborform module enables the rapid creation of complex, pipelined text parsers.  For example, a common sequence of NLP tasks for a passage of text might be:
...
An alternative approach involves performing each task sequentially, feeding the results of one into the next:
...
Such an approach is a *pipeline*.  In a pipeline, each task is encapsulated, which not only simplifies the implementation of an individual task, it makes it easy to alter the pipeline with additional tasks:

```mermaid
flowchart LR
  trim --> psplit["paragraph split"]
  psplit --> ssplit(["sentence split"])
...
|:---------------|:-----------------------------------------------------|
| Connectors.Next | Calls an itorator and yields its result |
| Connectors.Sub | Calls an itorator and itereatively feeds its result as input itos to the next connection |
| Connectors.XXX | Calls an itorator for a given ito, and then passes the same ito to the next connection |
| Connectors.Children.Add | Calls an itorator for a given ito and adds the results to the given ito's children |
| Connectors.Children.AddHierarchical | Calls an itorator for a given ito and hierarchically adds the results to the given ito's children |
| Connectors.Children.Replace | Calls an itorator for a given ito and replaces its children with the results |
| Connectors.Children.Delete | Calls an itorator for a gien ito and deletes the results from the given ito's children |

## The Pipeline

To indicate flow from one itorator to a another, wrap the target itorator in a connection, and add it to the first's connections list:

```python
>>> itor_1 = arborform.Reflect()
>>> itor_2 = arborform.Trim()
>>> itor_1.connections.append(Connections.Next(itor_2))
```

In this case, data flow moves unconditionally from itor_1 to ito2_2:
...
```mermaid
flowchart TB
  itor["itorator(ito)"] --> clone["ito.clone()"]
  clone --> transform["itorator._transform(ito)"]
  transform -->|"for each ito"| cons["itorator.connections"]
  cons -->|"for each connection"| nc{"next?"}
  nc -->|True| connector
  nc ---->|False| yield["yield ito"]
  connector --> pred{"connector.predicate(ito)"}
  pred -->|True| itor2["connector.itorator"]
  itor2 --> itorTyp{"type(itorator)"}
  itorTyp -->|ChildrenConnector| eval_to_children
  
  transform
  pred -->|False| nc
  ```

itorator(ito) -> ito.clone() -> itorator._transform() -for each ito -> .connections -for each connection -> if connection.predicate(itor) then connection.itorator._transform()... -> c.posterator()


Ito flows through Transform, then Connections, then postorator...


### Connector

``Itorator`` objects are chained together via ``Connector`` objects, contain a target ``Itorator`` and an optional predicate.
...
Six concrete types of Connector are available:
...
Connectors govern the *flow* if itos from one itorator to another.

### Next

The ``Next`` connector takes an Ito output from one ``Itorator``, and feeds it to a second.  The results from the second are then yielded back by the first.

### Sub

The ``Sub`` connector takes an Ito output from one ``Itorator``, and feeds it to a second.  The results from the second then proceed, one at a time, throught he remainder of the first's connections list.
