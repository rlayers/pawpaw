...
| ``Connectors.Subroutine`` | 1. invokes ``f(cur)``, ignoring any result<br>2. Continues inner loop with ``cur`` | perform modifications (such as altering the .children collection) on cur without disrupting flow; transformations that result in a new Ito (such as cloning, splitting, etc.) are not carried forward |
