...
|:---------------|:-----------------------|:-----------------------|
| ``Connectors.YieldBreak`` | 1. yields ``f(cur)``<br>2. breaks out of inner loop for ``cur`` | Cannonical |
| ``Connectors.Assign`` | 1. invokes ``f(cur)`` and for each item...<br>2. ...yields the result of allowing it to complete the inner loop<br>3. breaks out of inner loop for ``cur`` | Connect to multiple end-points of a sub-flow |
| ``Connectors.Sub`` | 1. invokes ``f(cur)``<br> 2. continues inner loop with ``cur`` | perform a transformation to cur without disrupting flow |
| ``Connectors.Children.Add`` | 1. caches result of ``f(cur)`` and for each item in the result...<br>2. adds it to ``cur.children``<br>3. continues inner loop with ``cur`` | adding children |
| ``Connectors.Children.AddHierarchical`` |  1. caches result of ``f(cur)`` and for each item in the result...<br>2. adds it hierarchically to cur.children<br>3. continues inner loop with ``cur`` | adding children  hierarchically |
| ``Connectors.Children.Replace`` | 1. caches result of ``f(cur)``<br>2. calls ``cur.children.clear()``<br>3. calls ``cur.children.add(*cached_result)``<br>4. continues inner loop with ``cur`` | adding or replacing children |
| ``Connectors.Children.Delete`` | 1. caches result of ``f(cur)``<br>2. calls ``cur.children.delete`` for each item in cached_result<br>3. continues inner loop with ``cur`` | deleting children |

If an itorator has a ``.postorator``, the result the entire flow is wrapped in an iterator and passed to it.

### ``Connections.YieldBreak``
...
>>> con = Connections.YieldBreak(itor_desc)
...
The ``Assign`` connector is useful when designing a *sub-pipeline* that has multiple end-points and whose output needs to be sent to another itorator:

```mermaid
flowchart LR
  A["A()"] -->|"Connections.Assign"| sub-pipeline
...
>>> con = Connections.Children.Add(split_chars)
