...
Note: Using the ``Split`` itorator with ``BoundaryRetention.DISTINCT`` is useful if you want to extract itos and then downstream process the residuals.
...
The ``Gaps`` itorator allows you to return an itorator's results *as well as the* the 'non-results', which can be further downstream processed.  For example, you can repeatedly perform extractions against a given Ito, which the leftover part of each step feeding into the next extractor:
