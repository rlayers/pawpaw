...
The Pawpaw arborform module supports the rapid creation of complex, pipelined text parsers.  Arborform utilizes Python generators to achieve a reasonable mixture of performance and memory conservation, and is highly flexible and scalable for your needs.  The key classes in arborform are:

* ``Itorator``[^itorator_name]: Generator-based pipelined ``Ito`` transformations
* ``Connector``: Encapsulates connections between itorators
* ``Postorator``: Generator-based ``Ito`` iterable consolidation
...
flowchart TB
  ...
  itor["itorator(ito)"] --> clone["ito.clone()"]
  clone --> transform["itorator._transform(ito)"]
  transform -->|"(for each ito)"| connections
  connections --> nc{"next?"}
  nc -->|True| connector
  nc ---->|False| yield
  connector --> pred["connector.predicate(ito)"]
