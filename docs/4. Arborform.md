...
```python
>>> arborform.Itorator.wrap(lambda i: i.clone())  # warning: returns an Ito, which itself is a iterable series of 1-char long itos
>>> arborform.Itorator.wrap(lambda i: i.clone(),)  # ok: returns a tuple containing a single ito
>>> arborform.Itorator.wrap(lambda i: [i.clone()])  # ok: returns a list containing a single ito
```
...
Itorators are connected via ``Connector`` objects, which are stored in a list accessible via the ``.connections`` property:
