...
The ``Itorator`` class encapsulates a single step in a pipeline whereby you are able to perform an transformation on a given ``Ito``.  Some examples of the types of transformations you may wish to do are:

* Use ``.str_strip`` to shorten an ``Ito``
* Split one ``Ito`` into a sequence of one or more itos
* Filter out an ``Ito`` so that it doesn't show in the final output

The transformation occurs in the the ``.iter`` method, which is abstract on the base class ``Itorator``.  Several concrete ``Itorator`` classes are available for use:
...
Alternatively, you can wrap an arbitrary transformation action using the ``.wrap`` method:
...
>>> for r in itor.traverse(i):
...   print(r)
Hello,
world!
```

The ``.tag`` property is a convenience field that allows you to lable your itoators with arbiratry str values, in order to eassist in testing and development of code. 
...
Pawpaw's arborform supports rapid parser creation using a divide and conquer strategy composed of discrete, pipelined steps.  Steps are peformed by ``Itorator`` instances, which operate on an ``Ito`` sequence.  An ``Itorator``, at the most basic level, takes an ``Ito`` and transforms it into an ``Ito`` *sequence* via its ``._iter``: method
...
The  ``.traverse`` method serves starting point for a pipeline, taking a single ``Ito`` as input:
...
``Itorator`` objects are chained together via the ``.itor_sub``, ``.itor_next``, & ``.itor_children`` properties.  For unconditional chaining, you can simply set these properties to the ``Itorator`` object you wish to link to.  More complex chaining approaches are also possible - see the [Advanced Chaining](#advanced-chaining) section below for more details.)

### ``itor_sub``

The ``.itor_sub`` property is used to manage *sub-pipeline* flows.  Using sub-pipelines allows you to better encapsulate various sub-processes of your filter, and provides a single connection point for any downstream itorators.
...
>>> D_1.itor_next = D_2
>>> # Wrap A
>>> wrapped = Itorator.wrap(A)  # Itorator.wrap has overload for passing another Itorator
>>> wrapped.itor_next = Z
```
