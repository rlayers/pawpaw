...
Transformations are performed in the ``._iter`` method, which is abstract for the base class ``Itorator``.  Several concrete subtypes with ``._iter`` implementations are available:
...
You can implement new functionality by creating new class that derived from ``Itorator``.  Alternatively, you can wrap an arbitrary transformations  using the ``Itorator.wrap`` method:
...
Note that the return type for your wrapped transorformation is an ito **sequence**.  Even if your transformation alters the input ito, or creates a single new ito, be sure your return value is still a sequence:
...
The ``itorator`` also features a ``.tag`` property, which is a convenience field that allows you to assign arbirary labels in order to assist in testing and development of your pipelines.
