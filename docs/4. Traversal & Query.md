...
Because you might be any number of steps down the hiearchy when you find a matching node, you don't know the number of steps to get back to where you started.  You *might* be able to construct another filter and apply it to a subsequent ancestor axis:
...
The behavior of a subquery is analagous to bookmarking the current axis, performing an exploratory query for each node in it, and then filtering the axis to select only those nodes whose exploratory query yielded any results.  You can even use a subquery to filter based on *non-matches*, e.g.:
