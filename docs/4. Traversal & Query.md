...
The subqueries component of a phrase consists of one or more subquery expressions, grouped and separated using parentheses and the same logical operators indicated for filters:

```
subqueries := [not_operator] subquery [ combining_operator [not_operator] subquery ]...
```

A subquery is itself a plumule query, and must be contained in curly braces:

```
subquery := '{' query '}'
```
 
A subqueries component allows you to specified that the current axis must meet a given criteria, based on a relative query, without affecting the axis itself.

#### Examples

```python
'*{*[blah]}  # blah blah
'*{*[foo]{**{foo ^ goo}}  # blah blah
```
