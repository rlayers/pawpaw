...
be used as a hierarchical data store.  You can traverse down to child nodes via the ``.children``
property, and up to the parent node via the ``.parent`` property:
...
>>> itor = segments.itorator.Extract(re)
>>> root.children.add(*itor.traverse(segments.Ito(s)))
...
numbers of nodes.  For large collections or more complex traversals, ``ito-segments`` offers a fully
...
A phrase, in turn, consists of an *axis* along with an optional *filter* and/or *subquery*::
...
.. admonition:: Key Concept
   ...
   An axis always returns nodes *nearest* to *farthest* from the current node(s), unless reversed.
