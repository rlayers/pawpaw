...
class TestTypeChecking(_TestIto):
    @classmethod
    def def_cm_w_typehints(a: T_A, b: T_B, c: T_C) -> T_RET:
        return True
    
    @classmethod
    def def_cm_wo_typehints(a, b, c):
        return True
    
    def def_im_w_typehints(a: T_A, b: T_B, c: T_C) -> T_RET:
        return True
    
    def def_im_wo_typehints(a, b, c):
        return True
    ...
    def setUp(self) -> None:
        ...
        self.test_data = [
            ...
            TestData('class def', True, TestTypeChecking.def_cm_wo_typehints),
            TestData('instance def', True, self.def_im_w_typehints),
            TestData('instance def', True, self.def_im_wo_typehints),
            TestData('lambda', True, lam_w_typehints),
            TestData('lambda', True, lam_wo_typehints),
        ]
            
        def test_is_callable_exact(self):
            for ti in self.test_data:
                with self.subTest(type=ti.name, type_hints=ti.type_hints):
                    self.assertTrue(segments.type_chekcing.is_callable(ti.func, TYPESIG_EXACT)
            
        def test_is_callable_union_element(self):
            for ti in self.test_data:
                with self.subTest(type=ti.name, type_hints=ti.type_hints):
                    self.assertTrue(segments.type_chekcing.is_callable(ti.func, TYPESIG_UNION_ELEMENT)
            
        def test_is_callable_subtype(self):
            for ti in self.test_data:
                with self.subTest(type=ti.name, type_hints=ti.type_hints):
                    self.assertTrue(segments.type_chekcing.is_callable(ti.func, TYPESIG_SUBTYPE)
            
        def test_is_callable_wrong_count(self):
            for ti in self.test_data:
                with self.subTest(type=ti.name, type_hints=ti.type_hints):
                    self.assertFalse(segments.type_chekcing.is_callable(ti.func, TYPESIG_TOO_FEW)
                    self.assertFalse(segments.type_chekcing.is_callable(ti.func, TYPESIG_TOO_MANY)
            
        def test_is_callable_wrong_types(self):
            for ti in self.test_data:
                with self.subTest(type=ti.name, type_hints=ti.type_hints):
                    expected = not ti.type_hints or ti.name == 'lambda'  # type hints for lambdas don't show in annotaitons
